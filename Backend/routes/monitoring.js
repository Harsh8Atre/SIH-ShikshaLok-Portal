// routes/monitoring.js - Student monitoring and attendance routes
...
// Get students monitoring data for a session (Faculty/Admin only)
router.get('/students/:sessionId',
  authenticateToken,
    ...
    try {
      ...
      if (req.user.role === 'faculty' && !session.faculty.equals(req.user.userId)) {
        return res.status(403).json({
          error: 'Access denied',
          message: 'You can only monitor your own sessions'
        });
      }
      ...
      // Transform data for monitoring dashboard
      const studentsData = attendanceRecords.map(attendance => ({
        ...
        // Device info (for faculty reference)
        deviceInfo: {
      ...
      const sessionStats = await Attendance.aggregate([
        { $match: { session: mongoose.Types.ObjectId(sessionId) } },
        {
          $group: {
            ...
            highRiskStudents: {
              $sum: { $cond: [{ $in: ['$calculated.riskLevel', ['high', 'critical']] }, 1, 0] }
            },
...
// Get alerts for a session (Faculty/Admin only)
router.get('/alerts/:sessionId',
  authenticateToken,
  requireRole(['faculty', 'admin']),
  validationRules.sessionId,
  handleValidationErrors,
...
// Resolve an alert (Faculty/Admin only)
router.patch('/alerts/:sessionId/:alertId/resolve',
  authenticateToken,
  requireRole(['faculty', 'admin']),
  async (req, res, next) => {
    try {
      ...
      // Find the attendance record with the alert
      ...
      // Find and update the specific alert
  ...
  async (req, res, next) => {
    try {
      ...
      if (!attendance) {
        ...
        if (!isEnrolled) {
          return res.status(403).json({
            error: 'Not enrolled in this session'
          });
        }
    ...
    try {
      ...
      if (!sessionId || !activityType) {
        return res.status(400).json({
          error: 'Session ID and activity type are required'
        });
      }
      ...
      // Update device and network info if provided
      if (deviceInfo) {
      ...
      req.app.get('io')?.to(`session-${sessionId}`).emit('student_activity', {
        ...
        activityType,
        timestamp: new Date(),
        studentName: req.user.userData?.name
      });
...
// Get attendance summary for session (Faculty/Admin only)
router.get('/attendance/:sessionId/summary',
  authenticateToken,
  requireRole(['faculty', 'admin']),
  validationRules.sessionId,
  handleValidationErrors,
...
router.get('/attendance/student/:studentId/history',
  authenticateToken,
  async (req, res, next) => {
    try {
      ...
      if (req.user.role === 'faculty') {
        // Faculty can only see attendance for their sessions
